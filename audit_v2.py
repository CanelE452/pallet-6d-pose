...
어제는 4 asset 중 **0 개**만 파일 단위 대조가 가능했다.  `usd-core` 를 설치해
USD 2 개를 실제로 파싱했으므로 이제 **2/4** 다.  나머지 2 개(GLB)는 이 머신에
없으므로 UNRESOLVED 로 남긴다 — 이름이 CC 라이선스 인터넷 모델처럼 보인다는 것은
근거가 아니다.

canonicalization 규칙 (명시)
    병합     stage 안의 모든 UsdGeom.Mesh 정점을 하나로 합친다
    평행이동 centroid 를 원점으로
    스케일   최대 extent 로 나눠 1 로 정규화
    회전     정점 좌표 자체는 회전에 불변이 아니므로, **회전 불변 signature** 를
             따로 만든다 — centroid 거리 히스토그램(정점 순서에도 불변).
             정렬된 extent 비율도 축 치환에 불변이다.
"""
...
def rotation_invariant(v):
    """정점 순서·회전·평행이동·균일스케일에 불변인 형상 서명."""
    v = v - v.mean(0)
    s = np.linalg.norm(v, axis=1)
    s = s / max(s.max(), 1e-12)
    hist, _ = np.histogram(s, bins=BINS, range=(0.0, 1.0), density=False)
...
def main():
    ...
    for name, ptype in USED.items():
        hit = list(ROOT.rglob(name))
        hit = [h for h in hit if ".git" not in str(h)]
        if not hit:
    ...
    def compare(a, b):
        ...
        return {
            "exact_vertex_hash": a["canonical_vertex_sha256"] == b["canonical_vertex_sha256"],
            ...
            "ratio_L1": round(float(np.abs(
                np.array(a["dimension_ratios"]) - np.array(b["dimension_ratios"])).sum()), 4),
        }

    comparisons = {n: compare(target, s) for n, s in resolved.items()}
    exact = sum(1 for c in comparisons.values() if c["exact_vertex_hash"])
    rot = sum(1 for c in comparisons.values() if c["rotation_invariant_hash_equal"])

    audit = {
        "question": "BROAD 가 쓴 mesh 가 평가 대상 mesh 와 같은가",
        "canonicalization": {
            "merge": "stage 내 모든 UsdGeom.Mesh 정점 병합",
            "translation": "centroid 를 원점으로",
            "scale": "최대 extent 로 정규화",
            "rotation": "정점 좌표는 회전 불변이 아니므로 centroid 거리 "
                        "히스토그램(64 bin)을 회전·정점순서 불변 서명으로 사용. "
                        "정렬된 extent 비율은 축 치환에 불변",
        },
        "target": target,
        "resolved": resolved,
        ...
        "coverage": {"assets_used_by_broad": len(USED),
                     ...
                     "note_v1": "V1 에서는 0/4 였다. usd-core 설치로 2/4 가 됐다."},
        "EXACT_OVERLAP": exact,
        "ALIAS_OVERLAP": 0,
        ...
        "unresolved_note":
            "GLB 2 종은 파일 부재로 mesh 단위 대조 불가. 이름이 CC 인터넷 모델처럼 "
            "보인다는 것은 근거가 아니다. 렌더 머신에서 회수하면 닫힌다.",
    }
    ...
    md = ["# TARGET ASSET EXCLUSION AUDIT — V2 (mesh 수준)", "",
          ...
          "V1 은 4 asset 중 **0 개**만 파일 단위 대조가 가능했다. `usd-core` 를",
          "설치해 USD 2 개를 실제로 파싱했으므로 이제 **2/4** 다.", "",
          "## 서명", "", "```",
          f"{'asset':34}{'verts':>10}{'faces':>10}{'aspect':>9}{'thick':>8}"]
    md.append(f"{'TARGET pallet_full.obj':34}{target['vertices']:>10,}"
              f"{target['faces']:>10,}{target['footprint_aspect']:>9.3f}"
              f"{target['thickness_ratio']:>8.4f}")
    for n, s in resolved.items():
        md.append(f"{n + '  (' + s['pallet_type'] + ')':34}{s['vertices']:>10,}"
                  f"{s['faces']:>10,}{s['footprint_aspect']:>9.3f}"
                  f"{s['thickness_ratio']:>8.4f}")
    md += ["```", "", "## 대조 결과", "", "```"]
    for n, c in comparisons.items():
        md.append(f"{n}")
        md.append(f"  exact vertex hash 일치        {c['exact_vertex_hash']}")
        md.append(f"  회전불변 형상 hash 일치        {c['rotation_invariant_hash_equal']}")
        md.append(f"  vertex/face 수 일치           {c['vertex_count_equal']} / {c['face_count_equal']}")
        md.append(f"  형상 히스토그램 L1 거리        {c['shape_hist_L1']}  (0 이면 동일)")
        md.append(f"  치수비 L1 거리                {c['ratio_L1']}")
    md += ["```", "", "## 아직 닫히지 않은 것", "", "```"]
