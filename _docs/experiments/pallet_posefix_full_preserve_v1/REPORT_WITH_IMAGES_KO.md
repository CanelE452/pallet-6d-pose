# FULL + preservation — 완료 보고

실험을 끝냈으며 **핵심 성능의 순이득은 없었습니다.** 일부 정상 출력 보존은 개선됐지만 사전 성공 기준을 모두 충족하지 못했습니다. `INCONCLUSIVE`는 실험 미실행이나 GPU 문제라는 뜻이 아니라, 부분 개선만 있고 채택 기준은 충족하지 못했다는 뜻입니다.

| 항목 | 기존 FULL | FULL + preserve |
|---|---:|---:|
| PRIMARY PCK10 | 50.35% | 50.35% |
| 20–40px → ≤10px 복구 | 6/99 | 5/99 |
| BASE 정답 손실 ↓ | 17 | 16 |
| N2 정답 손실 ↓ | 31 | 31 |
| 합성 clean PCK10 | 91.35% | 92.24% |
| 합성 stress PCK10 | 67.71% | 67.41% |
| GREEN manual PCK10 | 79.15% | 79.44% |
| CLEAN17 PCK10 | 90.44% | 91.91% |
| CAD18 PCK10 | 62.50% | 66.91% |

성공 gate 8개 중 7개 통과. 실패한 항목은 합성 clean: BASE 94.02% 대비 −1.78pp로, 허용한 −1pp를 벗어났습니다. FULL에서 맞던9개를 잃고 틀리던9개를 얻어 PRIMARY 순정답 수는 같았습니다. FULL의 큰 오류 복구6개 중5개를 유지하고 새 큰 오류 복구는0개였습니다.

LoRA 때처럼 stress 복구가 크게 사라진 결과는 아닙니다. 다만 보존 항을 추가하는 것만으로 실사의 큰 코너 오차 복구가 늘지는 않았습니다. >40px 복구는 FULL/FP 모두0입니다. 다음 병목 후보는 보존보다는 큰 오류를 찾는 이미지 증거/좌표 후보의 한계이지만, 이것을 원인으로 확정한 것은 아닙니다.

다음 분기는 **representation/geometry 무학습 진단 하나**로 계획만 남깁니다. 우선 이미 저장된 큰 오류 사례에서 가시 코너의 증거와 현재 후보 위치를 비교하는 headroom 감사를 설계합니다. 새 모델·PnP 버전·student·λ 재탐색은 실행하지 않습니다. 별도 데이터 감사에서는 같은 일반 플라스틱 single-session32 / multi-session32 후보를 확보할 수 있음을 확인했으나, 서로 다른 기존 pseudo-target recipe를 먼저 통일해야 하므로 학습 준비 완료를 뜻하지 않습니다.

사람 visibility 검토는 `REVIEW_PENDING`. 현재 논문 최종 모델·최종 표는 변경하지 않았습니다. 지시문의 no-push 문구와 별개로, 사용자가 앞서 지정한 ‘결과·이미지 Markdown + push’ 방침에 따라 이 공개 보고서와 비교 이미지25장만 게시합니다. checkpoint·비공개 mapping·원본 감사/후보 manifest는 제외합니다.
