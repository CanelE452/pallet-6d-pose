"""diag_pass_all_overlay.py — overlay EVERY diag-pass frame per domain.

Reuses filter_domain_analysis/_full_s2.json (inference-free).
diag pass = filt_diag(kp)[0] (matches filter_combo_9kp.py counts: indoor 7 / outside 27 / night 10).
...
Saves individual overlays + a per-domain contact sheet grid.
"""
import json
import os
import sys
...
from filter_pr_camfacing import filt_diag  # noqa: E402
...
def main():
    ...
    for ln in open(os.path.join(ROOT, "data/_eval_sets/_exclude.txt")):
        ...
        if ln:
            ex.add(ln)
    base = os.path.join(ROOT, "data/pallet/eval_results/filter_combo_9kp/diag_pass_all")
    summary = {}
    for dom in DOMAINS:
        ...
        for rank, r in enumerate(passed):
            ...
            cv2.imwrite(fn, img)
            imgs.append(img)
        cols = 3 if len(imgs) <= 9 else (5 if dom == "outside" else 4)
        ...
        summary[dom] = {"n": len(imgs), "good": n_good, "bad": n_bad,
                        "sheet": sheet_path}
        print(f"[{dom}] diag-pass={len(imgs)}  good(<{GOOD_PX:.0f}px)={n_good}"
              f"  bad={n_bad}  sheet={sheet_path}")
