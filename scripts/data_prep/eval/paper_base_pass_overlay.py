"""paper_base_pass_overlay.py — overlay diag-pass AND fullkp-pass frames per domain
using the PAPER_BASE inference cache (_full_paper_base.json), inference-free.

NOTE: uses _full_paper_base.json (paper base model), NOT ft_s2.

Per frame:
  - predicted 9 keypoints: GREEN dots + GREEN index 0..8 (cyan cuboid edges)
  - GT projected_cuboid 8 corners: MAGENTA cuboid
  - top badge: order-free Hungarian 9kp reproj (px) + good(<10px) flag
Saves every pass frame individually + a per-domain contact sheet, for each filter.
"""
import json
import os
import sys
...
from filter_pr_camfacing import filt_diag, filt_fullkp  # noqa: E402
...
def overlay(rec):
    ...
    cv2.putText(img, f"[paper_base] 9kp reproj={e9:.1f}px  [{tag}]", (8, 22),
...
def main():
    fp = os.path.join(ROOT, "data/pallet/eval_results/filter_domain_analysis/_full_paper_base.json")
    ...
    for ln in open(os.path.join(ROOT, "data/_eval_sets/_exclude.txt")):
        ...
        if ln:
            ex.add(ln)
    base = os.path.join(ROOT, "data/pallet/eval_results/filter_combo_9kp/paper_base_pass")
    summary = {}
    for fname, fpass in FILTERS.items():
        summary[fname] = {}
        for dom in DOMAINS:
            outdir = os.path.join(base, fname, dom)
            ...
            sheet_path = os.path.join(base, fname, f"{dom}_contact_sheet.jpg")
            if sheet is not None:
                cv2.imwrite(sheet_path, sheet)
            summary[fname][dom] = {"n": len(imgs), "good": n_good, "bad": n_bad,
                                   "dir": outdir, "sheet": sheet_path}
            print(f"[{fname}/{dom}] pass={len(imgs)}  good(<{GOOD_PX:.0f}px)={n_good}"
                  f"  bad={n_bad}")
