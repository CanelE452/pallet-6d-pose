"""paper_s2_filterval_passfail_overlays.py — Stage B (net_epoch_0057) on REAL
filterval (outside44 + night43 + manual36 = 123), split each frame into
pass / fail / underdet by the DEPLOYABLE (GT-free) filter AND, and save a
per-frame overlay image (pred kp = blue, PnP cuboid = red) into
per-domain pass/fail/underdet folders.

pass = f1_peak & f2_peak_ratio & f4_tta_stab & f5_rear_conf & f6_frsep & f7_posdepth
       (ALL AND, TAU = existing s1 values). f3_flip/f8_size_env/f9_bbox_iou EXCLUDED
       (f3 broken; f8/f9 need GT). NO GT is used for filtering or overlay.
underdet = n_det < 6 (pre-filter stage).

REUSE (no filter/decode logic reimplemented):
  T = paper_s2_testset17_9filters : build_rec / infer_squash / M(s1 helpers) / TAU /
      N_DET_MIN / E / WEIGHTS.
...
Usage: conda activate pallet-pose;
       python scripts/stage0/paper_s2/paper_s2_filterval_passfail_overlays.py
"""
...
def accept(r):
    """deployable AND over PASS_FILTERS (env-free filters only)."""
...
def save_overlay(ip, pred8, pred_c, proj_all, verdict, fid, n_det, failed, path):
    ...
    h2 = "pred=blue  PnP-cuboid=red  dims=(1.1,1.3,0.12)  GT-free filter AND"
...
def main():
    ...
    print(f"[filterval] {len(frames)} frames; dims={DIMS}; weights=Stage B ep0057; "
          f"squash-parity; pass=AND{PASS_FILTERS}")
