"""paper_s2_testset17_passfail_overlays.py — same GT-free pass/fail/underdet
overlay split as paper_s2_filterval_passfail_overlays (f3 EXCLUDED), but on the
hand-anno test set (cad11 + noapril6 = 17, stage22 manifest) instead of filterval.

Reuses the f3-excluded overlay module VERBATIM (O.accept / O.pnp_cuboid /
O.save_overlay / O.PASS_FILTERS / O.OUT_ROOT) and T.build_rec / T.infer_squash;
only the frame source (stage22 manifest) and domains (cad, noapril) change.
Adds cad/noapril subfolders under the existing filterval_passfail root.

pass = f1 & f2 & f4 & f5 & f6 & f7 (f3/f8/f9 excluded). underdet = n_det<6.
NO GT used for filtering or overlay (json read only for K).

Usage: conda activate pallet-pose;
       python scripts/stage0/paper_s2_testset17_passfail_overlays.py
"""
...
import paper_s2_filterval_passfail_overlays as O  # noqa: E402 (f3-excluded overlay)
...
def main():
    ...
    print(f"[testset17] {len(frames)} frames; dims={O.DIMS}; weights=Stage B ep0057; "
          f"squash-parity; pass=AND{O.PASS_FILTERS}")
