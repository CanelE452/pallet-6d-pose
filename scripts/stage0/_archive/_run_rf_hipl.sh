...
for K in combined outside night noapril; do
  PL=${PLDIR[$K]}; OUT="/home/minjae/Documents/github/pallet-pose/weights/paper_s2/paper_s2_rf_hipl/r1_$K"; mkdir -p "$OUT"
  echo "=== TRAIN hipl R1_$K (PL=$PL, PL ratio=50%) ==="
  ...
  python -u train.py \
    ...
    --balance_groups "mixed_v8_train|v4_split_base|aug_squash_v2|aug_trunc_v2|aug_scale_v2:30,paper_4pallet_mask_v1:20,$PL:50" \
    --mask_aux --mask_weight 0.01 --mask_warmup 0 --aspect_resize \
    --encoder_lr_scale 0.1 --encoder_freeze_steps 500 --trainable_scope all
  echo "=== DONE hipl R1_$K ==="
done
echo "ALL HIPL R1 DONE"
