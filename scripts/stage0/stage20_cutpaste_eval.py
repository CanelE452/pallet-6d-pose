"""stage20_cutpaste_eval.py — paired eval: control vs cut-paste (STAGE20 pilot).
...
Verdict: cut-paste improves REAR / honest reproj vs control -> "appearance gap
real, cut-paste works"; no change -> "geometry (not appearance) is the bottleneck
(consistent with STAGE16/19)"; worse -> "cut-paste also unfit".

Usage: conda run -n pallet-pose python scripts/stage0/stage20_cutpaste_eval.py \
...
OUT_DIR = os.path.join(ROOT, "data", "pallet", "eval_results", "stage20_cutpaste")
PAD = 100
THRESHOLD = 0.3
...
def main():
    ...
    ap.add_argument("--control", default=os.path.join(
        ROOT, "weights", "stage20_cutpaste_pilot", "control", "final_net.pth"))
    ap.add_argument("--cutpaste", default=os.path.join(
        ROOT, "weights", "stage20_cutpaste_pilot", "cutpaste", "final_net.pth"))
    args = ap.parse_args()
    ...
    inf_m = build_infer(args.cutpaste)
    ...
    for dom, fid, jp, ip in frames:
        ...
        inb = ((gt8[:, 0] >= 0) & (gt8[:, 0] < w0) &
               (gt8[:, 1] >= 0) & (gt8[:, 1] < h0))
        v8 = bool(inb.sum() == 8)
        for name, pr in (("control", pc), ("cutpaste", pm)):
    ...
    lines = []
    lines.append(f"STAGE20 CUT-PASTE PAIRED EVAL (both-detect n={npair}; "
                 f"filter-val+manual, SEALED)")
    lines.append(f"control  = {args.control}")
    lines.append(f"cutpaste = {args.cutpaste}")
    lines.append("")
    hdr = (f"{'arm':<9}{'n':>4}{'front_med':>10}{'rear_med':>10}"
           f"{'reproj_med':>11}{'good<10':>8}{'gross>20':>9}")
    lines.append(hdr); lines.append("-" * len(hdr))
    res = {}
    for name in ("control", "cutpaste"):
    ...
    rear_imp = better(res["cutpaste"]["rear_med"], res["control"]["rear_med"])
    rep_imp = better(res["cutpaste"]["reproj_med"], res["control"]["reproj_med"])
    front_reg = (res["cutpaste"]["front_med"] is not None
                 and res["control"]["front_med"] is not None
                 and res["cutpaste"]["front_med"] > res["control"]["front_med"] + 1.0)
    v8_reg = (resv["cutpaste"]["front_med"] is not None
              and resv["control"]["front_med"] is not None
              and resv["cutpaste"]["front_med"] > resv["control"]["front_med"] + 1.0)
    if (rear_imp or rep_imp) and not front_reg and not v8_reg:
        verdict = ("SIGNAL (cut-paste improves rear/honest w/o full-view "
                   "regression) -> appearance gap real, full candidate")
    elif front_reg or v8_reg:
        verdict = ("NEGATIVE+REGRESSION (cut-paste hurts full-view/front) -> "
                   "cut-paste unfit / boundary artifact")
    else:
        verdict = ("HONEST NEGATIVE (no rear/honest improvement) -> geometry "
                   "(not appearance) is the bottleneck, consistent w/ STAGE16/19")
    lines.append("")
    lines.append(f"VERDICT: {verdict}")
    lines.append("[caveat] small real GT (paired same-frame). cut-paste keeps "
                 "SYNTHETIC GT (label noise 0); only bg appearance real. real "
                 "pallets present in bg (unlabeled) = known contaminant.")

    txt = "\n".join(lines)
    print(txt)
    open(os.path.join(OUT_DIR, "cutpaste_paired.txt"), "w").write(txt)
    json.dump({"npair": npair, "control": res["control"],
               "cutpaste": res["cutpaste"], "v8": resv, "verdict": verdict,
               "res_full": res, "res_v8": resv},
              open(os.path.join(OUT_DIR, "cutpaste_eval.json"), "w"), indent=2)
    print(f"\n[save] {os.path.join(OUT_DIR, 'cutpaste_paired.txt')}")
