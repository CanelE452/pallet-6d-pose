"""All194 comparisons for continuation/diversity, using real saved predictions."""
import argparse
import html
...
from . import dino_source_diversity as S
...
def main(phase):
    X={'dino_visual_long':L,'dino_source_diversity':S}[phase];X.verify()
...
<p>왼쪽 R0 / 가운데 합성만 SYN / 오른쪽 합성+기존 수도레이블 MIX. 초록 G=GT, 청록 P=실제 출력 채널.
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('phase',choices=['dino_visual_long','dino_source_diversity']);main(p.parse_args().phase)
