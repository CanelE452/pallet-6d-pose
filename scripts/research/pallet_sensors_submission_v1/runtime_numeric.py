...
from runtime_numeric_checks import coordinates,choice,self_test,CROP_ATOL
DM=import_old('direct_inference');DM.DOC=OLD_DOC;DM.RAW=OLD_RAW
DirectInference=DM.DirectInference
...
def run():
    ...
    historical=read(OLD_DOC/'RUNTIME_PROTOCOL.json');plan=dict(historical,models=historical['models']+['PRIOR1','PRIOR2','PRIOR3'],cpu_threads_scope='Torch intra-op; actual OpenCV and Torch inter-op thread settings are separately recorded after model initialization',runtime_implementation=bound(HERE/'submission_runtime.py'),prior_dtype='RGB+Gaussian prior FP32, cuDNN enabled, TF32 disabled inside prior only',reason='Same-session accuracy-cost comparison after adding separately computed prior backbone; historical timing retained');freeze(DOC/'RUNTIME_PROTOCOL.json',plan);er=old('evaluate_real');pe=old('paper_evaluation');_,cache,_=er.baseline_inputs(LINE)
    ...
    assert self_test()
    freeze(DOC/'RUNTIME_NUMERIC_AMENDMENT.json',dict(original_protocol=bound(DOC/'RUNTIME_PROTOCOL.json'),diagnostic=bound(DOC/'RUNTIME_FLOAT_DIAGNOSTIC.json'),implementation=bound(HERE/'runtime_numeric.py'),checks=bound(HERE/'runtime_numeric_checks.py'),absolute_crop_tolerance=CROP_ATOL,relative_tolerance=0,tolerance_source=bound(HERE/'validate_prior.py'),reason='Fixed-input GPU repeats first differ in up1; exact corrected-prior replay is not guaranteed with the frozen nondeterministic cuDNN settings. No model/data/selection change.',pose_policy='Historical MAIN parity checked with historical points; current hypothesis and coverage must match; current numerical pose deltas retained, not declared bit-exact.',first_attempt_retained=True))
    models={'R0':er.PlainBaseline(R0)};records=[];params={};prior={};memory=[];numerical=[]
    attempt_file=RAW/'runtime'/('attempt_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')+'.json')
    def save_attempt(status):write(attempt_file,dict(status=status,protocol=bound(DOC/'RUNTIME_PROTOCOL.json'),before=status_before,records=records,memory=memory,params=params,no_fastest_attempt_selection=True))
    status_before=status;save_attempt('STARTED')
    ...
    try:
        for a in ('P','L','D','PRIOR'):
            for s in (1,2,3):
                name=f'{a}{s}'
                models[name]=PointInference(s) if a=='P' else DirectInference(s) if a=='D' else PriorInference(s) if a=='PRIOR' else old('inference').PalletLinePoseInference(LINE/f'runs/image_line_only_seed{s}/last.pt',LINE/'SELECTION.json')
                path=BRAW/f'evaluation/{name}' if a=='P' else OLD_RAW/f'evaluation/{name}' if a=='D' else RAW/f'evaluation/{name}' if a=='PRIOR' else LINE/f'evaluation/image_line_only_seed{s}'
        ...
        thread_state=dict(torch_intraop=torch.get_num_threads(),torch_interop=torch.get_num_interop_threads(),opencv=cv2.getNumThreads(),OMP_NUM_THREADS=os.environ.get('OMP_NUM_THREADS'),MKL_NUM_THREADS=os.environ.get('MKL_NUM_THREADS'))
        cpu_hardware=json.loads(subprocess.check_output(['lscpu','-J'],text=True))
        for repeat in range(plan['repeats']):
            ...
            for name in order:
                ...
                for k in plan['keys']:
                    ...
                    if name=='R0':
                        ...
                        for c,r in zip(pred['candidates'],prior[name][k]['candidates']):
                            if name.startswith('PRIOR'):
                                numerical.append(dict(model=name,key=k,repeat=repeat,**coordinates(c,r,f'{name}/{k}/accuracy')))
                            else:er.exact(c['keypoints_xy'],r['keypoints_xy'],f'{name}/{k}/accuracy')
                    ...
                    if pose is not None:
                        ...
                        checked_pose=pose
                        if name.startswith('PRIOR'):
                            saved_candidates=prior[name][k]['candidates'];checked_pose=main_pose(saved_candidates,*metadata[k])
                            assert checked_pose is not None
                            assert choice(candidates,*metadata[k])==choice(saved_candidates,*metadata[k]),'Numerical change altered the PnP hypothesis'
                            numerical[-1].update(pose_translation_delta_cm=float(np.linalg.norm(pose[1]-checked_pose[1])*100),pose_rotation_matrix_max_abs=float(np.max(np.abs(pose[0]-checked_pose[0]))),hypothesis_exact=True)
                        rotation,translation,_=checked_pose;truth=gt[fid]
            ...
            save_attempt('BALANCED_BLOCK_'+str(repeat+1)+'_COMPLETE');print('RUNTIME_BLOCK_COMPLETE',repeat+1,flush=True)
    except BaseException:
        save_attempt('TECHNICAL_INTERRUPTION_RETAIN_PARTIAL_SAMPLES');raise
    finally:
    ...
    # The parameter-count loop's `head` otherwise retains the final 68M-parameter
    # prior after clearing models, contaminating every isolated-memory row.
    models.clear();del model,m,head,pred;gc.collect();torch.cuda.empty_cache();isolated_memory={}
    assert torch.cuda.memory_allocated()==0,'Release retained panel tensors before one-model memory accounting'
    for name in plan['models']:
        if name=='R0':single=er.PlainBaseline(R0)
        elif name.startswith('PRIOR'):single=PriorInference(int(name[5:]))
    ...
    save_attempt('COMPLETE_FULL_PANEL')
    write(DOC/'RUNTIME_PANEL.json',dict(complete=True,status='DESKTOP_ONLY_MEASURED',protocol_sha256=sha(DOC/'RUNTIME_PROTOCOL.json'),raw_attempt=bound(attempt_file),before=status,after=gpu(),threads=thread_state,cpu_hardware=cpu_hardware,summary=summary,records=records,memory=memory,isolated_memory=isolated_memory,params=params,
        accuracy_parity=True,accuracy_parity_scope='R0/P/D/L bit-exact; PRIOR corrected corners numerically equivalent under existing absolute crop bound, center/boxes/scores exact',numeric_amendment=bound(DOC/'RUNTIME_NUMERIC_AMENDMENT.json'),numerical_replay=numerical,canonical_MAIN_pose_parity=True,canonical_MAIN_pose_parity_scope='Historical coordinates reproduce canonical saved metrics exactly; current prior coordinates preserve hypothesis/coverage with separately recorded numerical pose differences',all_models_resident_for_balanced_order=True,memory_scope='allocated CUDA bytes; incremental workspace, not independent process total VRAM',
