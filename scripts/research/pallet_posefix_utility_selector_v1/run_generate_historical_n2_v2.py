...
CORRECTION = P.DOC / 'N2_RUNTIME_CORRECTION_V2.json'
...
def historical_n2(head, data, row, original_points, variant, raw_hw, gain, offset):
    ...
    # Recreate the historical CPU light_batch geometry, not the GPU norm.
    box=torch.from_numpy(np.array(data.arrays['boxes'][[row]],copy=True))
    box_valid=torch.isfinite(box).all(-1)&(box[:,2:]>box[:,:2]).all(-1)
    box=torch.where(box_valid[:,None],box,box.new_tensor([0,0,1,1]))
    diagonal=(box[:,2:]-box[:,:2]).norm(dim=-1).clamp_min(1)
    single['candidate_displacements']=diagonal[:,None,None]*head.displacements.detach().cpu()[None]
    if variant == 0:
    ...
    if rule['lam'] != 0:
        result[changed]=(np.asarray(original_points,dtype=np.float64)[changed]-100+delta[changed]/gain)+100
    if variant == 0:
...
def main():
    P.verify()
    # Preserve the incomplete v1 chunks; numerical geometry differs for stress too.
    P.RAW=P.RAW/'source_cpu_geometry_v2'
    ...
    receipt_path=P.DOC/'N2_RUNTIME_COMPATIBILITY_V2_COMPLETE.json'
    ...
    receipt=dict(complete=True,source_complete=P.bound(P.DOC/'SOURCE_COMPLETE.json'),
                 ...
                 N2_decoder='CPU including CPU box norm/displacements and raw-before-padding arithmetic, original fixed rule',
