...
def prepare(run, seed=1):
    ...
    frozen_write(run/'RAW_CACHE_PARITY_PROTOCOL.json', result)
    return result
...
def execute(run, seed=1, device='cuda:0'):
    run = Path(run).resolve()
    pre = C.read(run/'RAW_CACHE_PARITY_PROTOCOL.json')
    if pre != prepare(run, seed):
        raise ValueError('Parity protocol changed')
    target = run/'RAW_CACHE_PARITY.json'
    if target.exists():
    ...
    input_sha = {**pre['input_sha256'], **pre['input_array_sha256'],
        str(run/'RAW_CACHE_PARITY_PROTOCOL.json'): C.sha(run/'RAW_CACHE_PARITY_PROTOCOL.json'),
    ...
    out_dir = run/'provenance/raw_cache_parity'
    out_dir.mkdir(parents=True, exist_ok=True)
    state_before = predictor.capture.state_sha()
    try:
...
if __name__ == '__main__':
    ...
    if args.phase in ('run', 'all'):
        ...
        print(args.run_dir.resolve()/'RAW_CACHE_PARITY_PROTOCOL.json')
