...
def summarize():
    with C.exclusive('annotating'):
        selected=rows();saved=0;clicks=0
        for r in selected:
            path=WORK/'annotations'/((C.ROOT/r['image']['path']).stem+'.json')
            if not path.exists():continue
            d=C.read(path)
            assert d['schema']=='hard_direct_clicks_v1' and d['image']==r['image']
            assert d['selection_sha256']==C.sha(C.DOC/'HARD_SELECTION_LOCK.json')
            assert d['PnP_used'] is False and d['pose'] is None and d['manual_kps'][8] is None
            for p,a in zip(d['manual_kps'][:8],d['keypoint_annotations'][:8]):
                if p is not None:assert a['source']=='manual_click'
            saved+=1;clicks+=sum(p is not None for p in d['manual_kps'][:8])
