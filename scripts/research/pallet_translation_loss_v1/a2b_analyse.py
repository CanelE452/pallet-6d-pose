...
def main() -> int:
    ...
    common = sorted(set(idx["A2B_CONTROL"]) & set(idx["A2B_LC"]))
    d = {k: np.array([idx["A2B_LC"][f][k] - idx["A2B_CONTROL"][f][k] for f in common])
         for k in ("translation_error_cm", "depth_error_cm", "rotation_error_deg", "corner2d_px")}
    report["paired_A2BLC_minus_CONTROL"] = {
        "n_common": len(common),
        **{k: {"median": float(np.median(v)), "mean": float(v.mean()),
               "frac_improved": float((v < 0).mean())} for k, v in d.items()}}
    sess = sorted({idx["A2B_CONTROL"][f]["session_id"] for f in common})
    report["paired_by_session"] = {
        s: {"n": int(sum(idx["A2B_CONTROL"][f]["session_id"] == s for f in common)),
            "delta_t_median_cm": float(np.median([idx["A2B_LC"][f]["translation_error_cm"]
                                                  - idx["A2B_CONTROL"][f]["translation_error_cm"]
                                                  for f in common
                                                  if idx["A2B_CONTROL"][f]["session_id"] == s])),
            "delta_depth_median_cm": float(np.median([idx["A2B_LC"][f]["depth_error_cm"]
                                                      - idx["A2B_CONTROL"][f]["depth_error_cm"]
                                                      for f in common
                                                      if idx["A2B_CONTROL"][f]["session_id"] == s]))}
        for s in sess}

    # ---- sigma escape 대리지표: keypoint confidence 분포 (§23, 기술통계만)
    conf = {}
    for arm in ("A2B_CONTROL", "A2B_LC"):
        p = json.loads((CLOSURE / "predictions" / f"{arm}.json").read_text())["frames"]
        v = np.array([c for fr in p.values() if fr.get("keypoints_conf")
                      for c in fr["keypoints_conf"][:8]], float)
        conf[arm] = {f"p{q}": float(np.percentile(v, q)) for q in (10, 50, 90)}
        conf[arm]["n"] = int(v.size)
    report["keypoint_confidence_proxy_for_sigma"] = conf
    report["sigma_note"] = ("kpts_sigma 를 직접 저장하지 않았다.  keypoint confidence 는 "
                           "대리지표이며 기술 통계로만 본다 (§23: 사후 임계 금지).")

    (OUT / "A2B_PER_FRAME.json").write_text(json.dumps(per_arm, indent=1, sort_keys=True) + "\n")
    (OUT / "A2B_SCREEN_ANALYSIS.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
    print(f"wrote {(OUT / 'A2B_SCREEN_ANALYSIS.json').relative_to(REPO)}")
    return 0
