"""Matched timing of all21 completed new/reference models; strict failures retained."""
...
import torch
import cv2
from scripts.research.pallet_dht_coupling_v2.evaluate import CanonicalPredictor, population, E, ROOT
from scripts.research.pallet_dht_coupling_v2.prepare import CONTROL_ARMS, OLD
...
def main():
    ...
    arms=list(CONTROL_ARMS)+protocol['arms']
    if len(arms)!=7 or len(set(arms))!=7:raise ValueError('Expected3 reference+4 new arms')
    ...
    for name in ['evaluate.py','integration.py']:
    ...
    for name in ['FINDINGS.json','OPERATOR_ISOLATION.json','SUMMARY.json']:
        path=OLD/'provenance/runtime_parity_diagnostic'/name;sources[str(path)]=sha(path)
    for seed in protocol['training']['seeds']:
        predictors={};references={};values={}
        for arm in arms:
            owner=OLD if arm in CONTROL_ARMS else root
            folder=owner/'evaluation'/f'{arm}_seed{seed}'
            c=read(folder/'COMPLETION.json')
            if not(c['complete'] and c['PASS']):raise ValueError('Accuracy evaluation unfinished')
            cp=owner/'runs'/f'{arm}_seed{seed}'/'weights/final.pt'
        ...
        for repeat in range(3):
            for i,(image,key) in enumerate(zip(images,keys)):
                order=arms if (i+repeat)%2==0 else list(reversed(arms))
                for arm in order:
        ...
        for arm,v in values.items():
            ...
            rows.append(dict(arm=arm,seed=seed,reference_model=arm in CONTROL_ARMS,n=len(v),median_ms=float(np.median(times)),
    ...
    result=dict(complete=True,timing_collection_complete=True,PASS=strict_ok,parity_PASS=strict_ok,
                n_models=21,n_timing_observations=1638,reference_models_retimed=9,new_models_timed=12,
                status='COMPLETE' if strict_ok else 'COMPLETE_WITH_STRICT_PARITY_FAILURE',
