...
V1 = ROOT / "scripts/research/pallet_dht_joint_v1"
...
def source_bindings(pair):
    ...
    for name in ["integration.py", "train.py", "gradient_surgery.py", "incidence.py"]:
        if (HERE / name).is_file():
            paths.append(HERE / name)
    paths.extend(V1 / name for name in ("evaluate.py", "integration.py", "train.py", "hough_block.py", "line_targets.py"))
...
def register_model_modules():
    """Register the v2 package pickle class before loading the unchanged v1 geometry."""
    return importlib.import_module("scripts.research.pallet_dht_coupling_v2.integration")
...
def checkpoint_metadata(path, arm, seed):
    ...
    for model in (value.get("ema"), value.get("model")):
        if model is not None:
            require(getattr(model, "coupling_arm", None) == arm and getattr(model, "joint_arm", None) == "hough_joint",
                    "Checkpoint coupling identity or unchanged joint forward differs")
...
def completed_training_binding(checkpoint, meta, protocol_path, source_sha):
    ...
    require(meta["expected_optimizer_steps"] == meta["optimizer_steps"] == 6998, "Registered main optimizer budget must be6998")
    require(meta["expected_epochs"] == 2, "Registered main epoch budget must be2")
    ...
    R0_PATH = register_model_modules().R0_PATH
...
def make_binding(args, pair):
    ...
    return dict(schema="pallet_dht_coupling_evaluation_protocol_v2", arm=args.arm, seed=args.seed,
...
def infer(destination, binding, pair):
    ...
    write(destination / "LINE_EVIDENCE.json", dict(complete=True, arm=binding["arm"], seed=binding["seed"],
        ...
        role_supervision="All four coupling methods retain the twelve-role line targets; only registered training loss/gradient processing differs from v1 joint."))
    if binding["arm"] in ARMS:
...
def pose_predictions(destination, payload, binding):
    ...
    freeze(destination / f"predictions/{name}.json", dict(schema_version="pallet_dht_coupling_pose_predictions_v2",
...
def metrics(destination, binding, bootstrap=False):
    ...
    write(destination / "RESULTS.json", dict(schema="pallet_dht_coupling_evaluation_v2", complete=True, PASS=True,
    ...
    write(destination / "COMPLETION.json", dict(schema="pallet_dht_coupling_evaluation_completion_v2", complete=True,
...
def main():
    ...
    parser.add_argument("--arm", choices=ARMS, default="balanced")
    ...
    require((args.run_dir / "PURPOSE.md").is_file(), "Purpose-declared v2 output root required")
