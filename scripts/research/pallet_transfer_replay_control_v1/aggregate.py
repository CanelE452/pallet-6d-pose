...
def write_rows(path,rowsets,source=False):
    fields=(['model','frame_id','kind','session_or_scenario','denominator','pck5_numerator','pck10_numerator','pck20_numerator',
             'matched','score','target_iou','errors_px_json'] if not source else
            ['model','frame_id','session_or_scenario','denominator','pck5_numerator','pck10_numerator','pck20_numerator',
             'matched','score','target_iou','errors_px_json'])
    temporary=path.with_suffix('.pending.csv')
    with temporary.open('w',newline='') as handle:
        writer=csv.DictWriter(handle,fieldnames=fields);writer.writeheader()
        for model in NAMES:
            for frame_id,row in sorted(rowsets[model].items()):
                value=dict(model=model,frame_id=frame_id,session_or_scenario=row.get('scenario') or row.get('session'),
                    denominator=row['denominator'],pck5_numerator=row['numerators']['5.0'],
                    pck10_numerator=row['numerators']['10.0'],pck20_numerator=row['numerators']['20.0'],
                    matched=int(row['matched']),score=row['score'],target_iou=row['target_iou'],
                    errors_px_json=json.dumps(row['errors_px'],separators=(',',':')))
                if not source:value['kind']=row['kind']
...
def main():
    ...
    write_rows(DOC/'RESULTS_PER_FRAME.csv',{n:read(RAW/'evaluation'/n/'TARGET_PER_FRAME.json') for n in NAMES})
    write_rows(DOC/'SOURCE_RESULTS_PER_FRAME.csv',source_rows,source=True)
