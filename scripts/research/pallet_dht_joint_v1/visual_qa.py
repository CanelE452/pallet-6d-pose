...
desktop opening, or notification. A missing/incomplete nine-run experiment
...
def audit_saved_data(run_dir, inputs):
    ...
    require(render.get("complete") and render.get("PASS") and render.get("experiment_complete")
            and render.get("n_completed_evaluations") == 9 and render.get("n_gallery_frames") == 319,
            "Actual visual QA requires completed nine-run/319-frame report")
    ...
    exact(data["arms"], list(ARMS), "three arms")
    ...
    require(len(summary["runs"]) == 9, "Summary must contain all nine actual runs")
    ...
    exact(n_predictions, 319 * 9, "actual gallery prediction pairs")
    return data, dict(PASS=True, n_original_images=319, n_actual_runs=9, n_prediction_overlays=n_predictions,
...
def audit_browser(run_dir, data):
    ...
    try:
        ...
        exact(state, dict(title=TITLE, frames=319, current=CASE, arm="hough_joint", seed="1", role_count=12, frame_options=319), "actual browser default")
        checks.append("Default opens the required actual case with hough_joint seed1 and all319 frames")
        ...
        checks.append("All three arms and three seeds retain319 images; point-only has no invented Hough evidence")
        browser.select("arm", "hough_joint")
        browser.select("seed", 1)
        case = next(row for row in data["frames"] if row["id"] == CASE)
        affine = case["runs"]["hough_joint_seed1"]["evidence"]["affine"]
        ...
        browser.select("reference", "point_only")
        ...
        browser.select("role", 3)  # front_left_height, fixed before any outcome.
        ...
        return dict(PASS=True, default=state, decoded_embedded_images=decoded, checks=checks,
            ...
            required_case=CASE, captured_role=3, captured_role_name="front_left_height",
...
def audit(run_dir):
    ...
    result = dict(schema="pallet_dht_joint_actual_visual_qa_v1", complete=True, PASS=True,
        created_at_utc=datetime.now(timezone.utc).isoformat(), html_sha256=sha(run_dir / "index.html"),
        experiment_complete=True, n_completed_evaluations=9, n_gallery_frames=319, broken_local_images=0,
        ...
        output_sha256=browser["screenshots"], source_sha256={str(Path(__file__).resolve()): sha(__file__), str(HERE / "line_targets.py"): sha(HERE / "line_targets.py")},
