...
분리된 모듈:
  ...
  annotate_sessions.py  session discovery / metadata / safe path validation
  annotate_navigation.py  frame-slider coordinate conversion
...
from annotate_navigation import (
    _FRAME_SLIDER_TICKS,
    _frame_cur_to_tick,
    _frame_tick_to_cur,
    _frame_trackbar_target,
)
from annotate_sessions import (
    _ANNOTATION_IMAGE_SUFFIXES,
    _INCOMING_FRAME_REVIEW_FIELDS,
    _INCOMING_FRAME_REVIEW_LABELS,
    _LEGACY_DEFAULT_K,
    _OUT_ALIAS,
    _SEALED_SESSIONS,
    _SESSION_RUNTIME_ARG_FIELDS,
    _annotation_intrinsics_consensus,
    _configuration_mismatch,
    _direct_child_name,
    _discover_evaluation_session_pool,
    _explicit_text,
    _incoming_reviewed_frame_partitions,
    _metadata_value,
    _path_is_within,
    _require_legacy_read_dir,
    _require_nonlegacy_output_dir,
    _resolve_annotation_configuration,
    _resolve_legacy_read_dir,
    _resolve_session_intrinsics,
    _session_entry_parts,
    _session_image_paths,
...
def _annotated_session_frames(s, out_json):
    ...
    if getattr(s, "active_evaluation_member", True):
        try:
            _validate_evaluation_paths(
                eval_root,
                source_session_dir,
...
def _refresh_evaluation_workspace(
    s,
    annotation_path,
    *,
    image_path=None,
    ...
    try:
        summary = refresh_after_annotation(
            ...
            synchronize_session=bool(synchronize_session),
        )
    except Exception as exc:
...
def main(argv=None):
    ...
    def load_session(i):
        ...
        print(f"\n[Session {i+1}/{len(sessions)}] {nm}"
              ...
              f"{'   [STAGING · 저장 시 통합 진행률 반영]' if staging else ''}")
        print(f"           {len(sel)} frames (stride={args.stride}) of {len(rp)}")
        print(f"           Output: {od if writable else 'NONE (review only)'}")
        if staging:
            print("           Save scope: object-specific staging; the saved frame "
                  "is synchronized to combined evaluation progress")
