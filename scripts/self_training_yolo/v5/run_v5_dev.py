"""V5 development driver — 학습에서 끝내지 않는다.
...
METHOD_LOCK = V5_RESULTS / "RELIABILITY_SCORE_LOCK.json"
DATASETS = REPO_ROOT / "challenge/yolo_pose_one_model/datasets/paper_selftrain_v5"
...
def assert_contract_passed() -> None:
    receipt = REPO_ROOT / "data/pallet/results/paper_selftrain_v3/TRUE_IGNORE_LOSS_CONTRACT.json"
...
def evaluate(name: str, weights: Path) -> tuple[Path, Path]:
    ...
    v3_dir = str(REPO_ROOT / "scripts" / "self_training_yolo" / "v3")
...
def main() -> int:
    ...
    epochs = 1 if args.stage == "smoke" else int(lock["training_recipe"]["epochs"])
    tag = args.stage.upper()

    state = {
        "schema_version": "v5_dev_driver_state_v1",
        "stage": args.stage,
        "started": datetime.now(timezone.utc).isoformat(),
        "epochs": epochs,
        "arms": {},
    }
    state_path = V5_RESULTS / f"V5_DRIVER_STATE_{tag}.json"
    ...
    for arm in args.arms:
        ...
        if outcome["status"].startswith("FAILED"):
            notify(f"V5 {tag} **중단** — {arm} {outcome['status']}")
            raise SystemExit(f"TRAINING_FAILED: {arm} {outcome['status']}")

    if args.stage == "smoke":
        notify(f"V5 smoke 통과 — {len(args.arms)} arm 각 1 epoch. full 로 넘어간다.")
        print("\nsmoke 통과.", flush=True)
        return 0
    ...
    metrics = subprocess.run(
        [sys.executable,
         str(REPO_ROOT / "scripts/self_training_yolo/v5/v5_dev_metrics.py"),
         "--tag", tag],
    ...
    if metrics.returncode != 0:
        ...
        notify(f"V5 {tag} — 지표 계산 실패")
        raise SystemExit("METRICS_FAILED")

    verdict = json.loads((V5_RESULTS / "V5_DEV_GATE.json").read_text())
    lines = [f"**V5 DEV {verdict['status']}**"]
