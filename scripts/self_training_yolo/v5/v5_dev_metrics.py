...
EVAL_OUT = REPO_ROOT / "data/pallet/results/paper_eval_v5/arms"
V2_RESULTS = REPO_ROOT / "data/pallet/results/paper_selftrain_v2"
V5_RESULTS = REPO_ROOT / "data/pallet/results/paper_selftrain_v5"
OUT_JSON = V5_RESULTS / "V5_DEV_METRICS.json"
GATE_JSON = V5_RESULTS / "V5_DEV_GATE.json"
OUT_MD = REPO_ROOT / "_docs/archive/paper_pre_final_20260903/legacy_paper_outputs/generated/V5_DEV_RESULTS.md"

# V2 결과는 읽기만 한다 (V2 평가 폴더에서).  V3 는 자기 폴더에 쓴다.
V3_EVAL = REPO_ROOT / "data/pallet/results/paper_eval_v3/arms"

ARMS = ("V5_RELIABILITY_WEIGHTED",)
BASE = "R0"
PROPOSED = "V5_RELIABILITY_WEIGHTED"
CONTROL = "V3B_TRUE_IGNORE_AMBIG__FULL"
V3_REFERENCE = "V3B_TRUE_IGNORE_AMBIG__FULL"
...
def main() -> int:
    ...
    runs = REPO_ROOT / "challenge/yolo_pose_one_model/paper_selftrain_v5"
    ...
    rows[V3_REFERENCE] = load_per_frame(V3_REFERENCE, V3_EVAL)
    checkpoints[V3_REFERENCE] = (
        REPO_ROOT / "challenge/yolo_pose_one_model/paper_selftrain_v3"
        / V3_REFERENCE / "weights" / "last.pt")

    predictions = dump_predictions(
        checkpoints, context, V5_RESULTS / "V5_AXIS_PREDICTIONS.json")

    report: dict = {
        "schema_version": "v5_dev_metrics_v1",
        "tag": args.tag,
        "n_frames": len(context),
        "models": {},
    }
    for name in (BASE, V3_REFERENCE, *ARMS):
        block = {
    ...
    # ── gate G1~G9 (§25).  기준은 lock 과 이 블록에 고정되어 있다 ──────
    proposed = report["models"][PROPOSED]
    base = report["models"][BASE]
    control = report["models"][V3_REFERENCE]

    def rate(block, key):
        entry = block["detection"].get(key)
        return None if entry is None else entry["rate"]

    def nme(block, domain):
        return block["paired_nme"][domain]["frame_delta"]["median"]

    def level(block, domain):
        return block["paired_nme"][domain]["arm_nme_median"]

    gates: dict[str, dict] = {}
    night_v5, night_v3 = rate(proposed, "nighttime"), rate(control, "nighttime")
    gates["G1_night_detection_holds"] = {
        "pass": bool(night_v5 is not None and night_v3 is not None
                     and night_v5 >= night_v3),
        "detail": f"V3-B {number(night_v3, '.3f')} -> V5 {number(night_v5, '.3f')}"}
    day_v5, day_r0 = rate(proposed, "daytime"), rate(base, "daytime")
    gates["G2_day_detection_no_collapse"] = {
        "pass": bool(day_v5 is not None and day_r0 is not None
                     and day_v5 >= day_r0 - DAY_DETECTION_TOLERANCE),
        "detail": f"R0 {number(day_r0, '.3f')} -> V5 {number(day_v5, '.3f')} "
                  f"(허용 -{DAY_DETECTION_TOLERANCE})"}
    gates["G3_all_nme_below_r0"] = {
        "pass": bool(nme(proposed, "ALL") is not None and nme(proposed, "ALL") < 0),
        "detail": f"Δframe median {number(nme(proposed, 'ALL'), '+.5f')} (음수여야 한다)"}
    gates["G4_night_nme_not_worse_than_r0"] = {
        "pass": bool(nme(proposed, "nighttime") is not None
                     and nme(proposed, "nighttime") <= 0),
        "detail": f"Night Δframe {number(nme(proposed, 'nighttime'), '+.5f')} (0 이하)"}
    v5_all, v3_all = level(proposed, "ALL"), level(control, "ALL")
    gates["G5_better_than_v3b"] = {
        "pass": bool(v5_all is not None and v3_all is not None and v5_all < v3_all),
        "detail": f"V3-B {number(v3_all)} -> V5 {number(v5_all)}"}
    v5_night, v3_night = level(proposed, "nighttime"), level(control, "nighttime")
    gates["G6_night_better_than_v3b"] = {
        "pass": bool(v5_night is not None and v3_night is not None
                     and v5_night < v3_night),
        "detail": f"Night V3-B {number(v3_night)} -> V5 {number(v5_night)}"}
    axis_v5, axis_r0 = proposed["axis"]["all"]["rate"], base["axis"]["all"]["rate"]
    gates["G7_axis_at_most_r0"] = {
        "pass": bool(axis_v5 is not None and axis_r0 is not None and axis_v5 <= axis_r0),
        "detail": f"R0 {number(axis_r0, '.3f')} -> V5 {number(axis_v5, '.3f')}"}
    amb_v5 = proposed["axis"]["ambiguous"]["rate"]
    amb_r0 = base["axis"]["ambiguous"]["rate"]
    gates["G8_ambiguous_axis_at_most_r0"] = {
        "pass": bool(amb_v5 is not None and amb_r0 is not None and amb_v5 <= amb_r0),
        "detail": f"q>=0.75  R0 {number(amb_r0, '.3f')} -> V5 {number(amb_v5, '.3f')}"}
    gates["G9_not_detection_only"] = {
        "pass": bool(gates["G3_all_nme_below_r0"]["pass"]
                     and gates["G4_night_nme_not_worse_than_r0"]["pass"]),
        "detail": "G1/G2 만 좋아지고 G3/G4 가 실패하면 overall FAIL"}

    status = "PASS" if all(item["pass"] for item in gates.values()) else "FAIL"
    verdict = {
        "schema_version": "v5_dev_gate_v1",
        "V5_METHOD_STATUS": "DEV_PASS" if status == "PASS" else "FAILED",
        "status": status,
        "tag": args.tag,
        "gates": gates,
        "note": ("PAPER_EVAL 은 V1~V4 와 FILTER_SEPARABILITY 에 이미 쓰였다.  PASS 여도 "
                 "DEV_PASS_REQUIRES_UNTOUCHED_CONFIRMATION 까지만 판정한다."),
    }
...
def render(report: dict, verdict: dict) -> None:
    ...
    lines += ["```", "", f"**{verdict['status']}** — "
              f"`V5_METHOD_STATUS = {verdict['V5_METHOD_STATUS']}`", "",
              verdict["note"], ""]
