"""V2 development driver — 학습에서 끝내지 않는다.

전역 규칙(`~/.claude/CLAUDE.md` "장시간 작업은 결과까지 자동으로") 을 따른다.

    STEP 1  arm 학습            완료 판정은 results.csv 의 epoch 수로만
    STEP 2  PAPER_EVAL 평가     V1 과 같은 evaluator·같은 manifest
    STEP 3  V2 지표 + gate      기준은 이 파일에 하드코딩 — 결과 보고 못 고친다
    STEP 4  알림                '완료' 가 아니라 **판정** 을 보낸다

V1 산출물은 읽기만 한다.  V2 는 전부 별도 경로에 쓴다.
"""
...
DATASETS = REPO_ROOT / "challenge/yolo_pose_one_model/datasets/paper_selftrain_v2"
...
# V1 과 동일.  V2 는 supervision contract 만 바꾼다 (§11).
TRAIN_ARGS = dict(
...
def notify(message: str) -> None:
    """디스코드로 판정을 보낸다.  실패해도 드라이버를 멈추지 않는다."""
...
def train_arm(arm: str, epochs: int, tag: str) -> dict:
    ...
    if available < 6.0:
        raise SystemExit(
            f"REFUSING_TO_START: 가용 메모리 {available:.1f} GB. "
            "고아 dataloader 가 남아 있는지 확인하라.")
...
def main() -> int:
    ...
    state = {
        "schema_version": "v2_dev_driver_state_v1",
        "stage": args.stage,
        "started": datetime.now(timezone.utc).isoformat(),
        "epochs": epochs,
        "arms": {},
    }
    state_path = V2_RESULTS / f"V2_DRIVER_STATE_{tag}.json"
    ...
    for arm in args.arms:
        ...
        print(f"{arm}: {outcome['status']}", flush=True)
        if outcome["status"].startswith("FAILED"):
            notify(f"V2 {tag} **중단** — {arm} {outcome['status']}")
            raise SystemExit(f"TRAINING_FAILED: {arm} {outcome['status']}")

    if args.stage == "smoke":
        notify(f"V2 smoke 통과 — {len(args.arms)} arm 각 1 epoch 학습 완료. "
               "full 로 넘어간다.")
        print("\nsmoke 통과.  full 로 넘어가라.", flush=True)
        return 0
    ...
    # R0 baseline 도 같은 출력 폴더에 둔다 (V1 결과를 재사용하지 않는다 —
    # 같은 코드 경로로 다시 잰다).
    evaluate("R0", R0)
    ...
    metrics = subprocess.run(
        [sys.executable,
         str(REPO_ROOT / "scripts/self_training_yolo/v2/v2_dev_metrics.py"),
         "--tag", tag],
        cwd=REPO_ROOT, text=True, capture_output=True)
    print(metrics.stdout[-6000:], flush=True)
    if metrics.returncode != 0:
        print(metrics.stderr[-3000:], flush=True)
        notify(f"V2 {tag} — 지표 계산 실패")
        raise SystemExit("METRICS_FAILED")

    verdict_path = V2_RESULTS / "V2_DEV_GATE.json"
    verdict = json.loads(verdict_path.read_text())

    # ── STEP 4 알림 = 판정 ────────────────────────────────────────────
    lines = [f"**V2 DEV {verdict['status']}**"]
