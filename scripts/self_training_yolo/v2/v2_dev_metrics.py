...
EVAL_OUT = REPO_ROOT / "data/pallet/results/paper_eval_v2/arms"
V2_RESULTS = REPO_ROOT / "data/pallet/results/paper_selftrain_v2"
OUT_JSON = V2_RESULTS / "V2_DEV_METRICS.json"
GATE_JSON = V2_RESULTS / "V2_DEV_GATE.json"
OUT_MD = REPO_ROOT / "_docs/archive/paper_pre_final_20260903/legacy_paper_outputs/generated/V2_DEV_RESULTS.md"

ARMS = ("V2A_CONF25", "V2B_KP_MASK", "V2C_AMBIG", "V2D_FULL")
BASE = "R0"
PROPOSED = "V2D_FULL"
CONTROL = "V2A_CONF25"
...
def load_per_frame(name: str) -> dict[str, dict]:
    path = EVAL_OUT / f"{name}_per_frame.csv"
    if not path.exists():
...
def main() -> int:
    ...
    runs = REPO_ROOT / "challenge/yolo_pose_one_model/paper_selftrain_v2"
    ...
    predictions = dump_predictions(
        checkpoints, context, V2_RESULTS / "V2_AXIS_PREDICTIONS.json")

    report: dict = {
        "schema_version": "v2_dev_metrics_v1",
        "tag": args.tag,
    ...
    # ── gate (§19).  기준은 위 상수와 이 블록에 고정되어 있다 ──────────
    ...
    gates["1_night_detection"] = {
        "pass": bool(night_prop is not None and night_base is not None
                     and night_prop >= night_base),
        "detail": f"R0 {number(night_base, '.3f')} -> V2-D {number(night_prop, '.3f')}",
    }
    day_base, day_prop = rate(base, "daytime"), rate(proposed, "daytime")
    gates["2_day_detection_no_collapse"] = {
        "pass": bool(day_prop is not None and day_base is not None
                     and day_prop >= day_base - DAY_DETECTION_TOLERANCE),
        "detail": f"R0 {number(day_base, '.3f')} -> V2-D {number(day_prop, '.3f')} "
                  f"(허용 -{DAY_DETECTION_TOLERANCE})",
    }
    nme = proposed["paired_nme"]["ALL"]
    gates["3_common_nme_below_r0"] = {
        "pass": bool(nme["frame_delta"]["median"] is not None
                     and nme["frame_delta"]["median"] < 0),
        "detail": f"Δframe median {number(nme['frame_delta']['median'], '+.5f')} "
                  f"(음수여야 한다)",
    }
    control_nme = control["paired_nme"]["ALL"]
    gates["4_common_nme_below_v2a"] = {
        "pass": bool(nme["arm_nme_median"] is not None
                     and control_nme["arm_nme_median"] is not None
                     and nme["arm_nme_median"] < control_nme["arm_nme_median"]),
        "detail": f"V2-A {number(control_nme['arm_nme_median'])} -> "
                  f"V2-D {number(nme['arm_nme_median'])}",
    }
    ambiguous_prop = proposed["axis"]["ambiguous"]["rate"]
    ambiguous_ctrl = control["axis"]["ambiguous"]["rate"]
    gates["5_ambiguous_axis_below_v2a"] = {
        "pass": bool(ambiguous_prop is not None and ambiguous_ctrl is not None
                     and ambiguous_prop < ambiguous_ctrl),
        "detail": f"q>=0.75  V2-A {number(ambiguous_ctrl, '.3f')} -> "
                  f"V2-D {number(ambiguous_prop, '.3f')}",
    }
    all_prop = proposed["axis"]["all"]["rate"]
    all_base = base["axis"]["all"]["rate"]
    gates["6_axis_at_most_r0"] = {
        "pass": bool(all_prop is not None and all_base is not None
                     and all_prop <= all_base),
        "detail": f"R0 {number(all_base, '.3f')} -> V2-D {number(all_prop, '.3f')}",
    }
    gates["7_not_detection_only"] = {
        "pass": gates["3_common_nme_below_r0"]["pass"],
        "detail": "detection 만 좋아지고 localisation 이 악화하면 FAIL — gate 3 과 같다",
    }

    status = "PASS" if all(item["pass"] for item in gates.values()) else "FAIL"
    verdict = {
        "schema_version": "v2_dev_gate_v1",
        "V2_METHOD_STATUS": "DEV_PASS" if status == "PASS" else "FAILED",
        "status": status,
        "tag": args.tag,
        "gates": gates,
        "note": ("DEV 는 방법 개발 모집단이다.  CI 가 0 을 포함해도 방향과 효과 크기로 "
                 "final 을 진행할 수 있으나, 이것을 paper confirmation 이라 부르지 "
                 "않는다."),
    }
