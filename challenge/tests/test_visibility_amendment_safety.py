...
def test_visibility_source_reason_are_allowed(apply_module):
    before = payload()
    after = copy.deepcopy(before)
    for point in after["objects"][0]["keypoint_annotations"]:
        point["visibility"] = 2
        point["source"] = apply_module.REVIEW_SOURCE
        point["reason"] = apply_module.REVIEW_REASON
...
def test_unknown_state_is_held_not_written(apply_module):
    ...
    assert apply_module.STATES["v"] == 2
    assert apply_module.STATES["o"] == 1
    assert apply_module.STATES["t"] == 0
