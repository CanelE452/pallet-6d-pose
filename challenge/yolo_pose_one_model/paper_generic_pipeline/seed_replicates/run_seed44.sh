#!/usr/bin/env bash
# YOLO26N_PAPER_GENERIC_V1_SEED44 — seed42 와 seed 만 다르다.
...
yolo pose train \
  ...
  seed=44 deterministic=True single_cls=True pretrained=True \
  ...
  project="$YR/runs_paper" name=yolo26n_paper_generic_v1_seed44 exist_ok=False

python $YR/paper_generic_pipeline/seed_replicates/compare_args.py \
  $YR/runs_paper/yolo26n_paper_generic_v1_seed44/args.yaml \
  || { "$NOTIFY" "❌ seed44 args 가 seed42 와 다르다"; exit 1; }
"$NOTIFY" "seed44 60ep 완료 — args seed42 와 동일 확인" || true
