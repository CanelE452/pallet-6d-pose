"""Extract and train the exact post-hoc Phase-E S0/S1 spatial probes.

This runner intentionally does not modify YOLO.  It freezes the completed
60-epoch ``last.pt``, captures the active one2one classification penultimate
7x7 patch at the postprocess-selected top candidate, then trains the original
S0/S1 probe class.  Existing Phase-C/E feature caches are never reused.
"""
...
def train(device: str) -> dict[str, Any]:
    ...
    for name, indices in (("TRAIN", train_indices), ("DEV", dev_indices)):
        if not bool(valid[indices].all()):
            raise RunnerError(
                f"mixed synthetic {name} contains "
                f"{int((~valid[indices]).sum())} invalid detection rows"
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("command", choices=("extract", "train", "all", "unit-smoke"))
    ...
    if args.command in {"extract", "all"}:
        print(json.dumps(extract(device), indent=2))
