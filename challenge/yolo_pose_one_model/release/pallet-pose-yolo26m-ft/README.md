...
This is the **medium** variant (23.6M params). A smaller sibling trained on identical data
with an identical recipe is at
[**CanelE452/pallet-pose-yolo26n-ft**](https://huggingface.co/CanelE452/pallet-pose-yolo26n-ft)
(nano, 2.7M params) — see [Which one should you use](#which-one-should-you-use), because
**the medium is not clearly better**.
...
model = YOLO("pallet_yolo26m_pose_ft.pt")
PAD = 100
...
|---|---|
| `imgsz` | 640 |
| `conf` | 0.4 (can go to 0.25, see [Tuning](#tuning-conf)) |
...
Evaluated on a held-out real set of 161 manually annotated frames **excluded from training**,
and on the 911-frame deployment sequence.
...
| **this model (medium ft)** | **0.0%** | **0.0%** | **0.0%** | **0.0%** |
...
| model | detection | keypoint err (median) | keypoint err (p90) |
|---|---|---|---|
| synthetic-only pretrain (nano) | 88.2% | 9.30 px | 28.41 px |
| nano finetune | 97.5% | 7.38 px | **26.75 px** |
| **this model (medium ft)** | 96.9% | **6.51 px** | 31.56 px |
...
Paired comparison on the 153 held-out frames both models detect:

```
medium more accurate   87 frames
nano   more accurate   66 frames
median difference   -0.24 px  (medium better)
mean   difference   +1.48 px  (medium worse — driven by its tail)
Wilcoxon p = 0.14   -> NOT statistically significant
```

Error-tail breakdown:

| threshold | nano | medium |
|---|---|---|
| > 10 px | 50 | **42** |
| > 15 px | 28 | **26** |
| > 20 px | **19** | 21 |
...
**The medium is better in the middle of the distribution and worse in the tail.** It wins
more frames than it loses, but not by a statistically significant margin.

Inference latency (RTX 3080, 640×640 padded, single frame):

| model | params | latency |
|---|---|---|
| nano | 2.7 M | 12.3 ms |
| medium | 23.6 M | 15.5 ms |

**Recommendation:** start with the **nano** — it is 8.7× smaller, has a tighter error tail,
and the accuracy difference is not significant. Take the medium if your platform has room
and you care about typical-case precision more than worst-case, or if you plan to finetune
further on more data (the larger model has more headroom).

---

## Known limitations

Both variants share these — they come from the data, not the architecture.

- **Heavily clipped pallets can be missed.** One deployment frame with a pallet cut off at
  the bottom edge went 0.427 (pretrain) → 0.000 (finetuned). Training longer does not fix
  it; this comes from the background-image composition, not from undertraining.
- **Distant, small pallets** are weaker for the same reason.
...
`patience=0` disables early stopping deliberately: with the default patience the nano run
stopped at epoch 16 and saved epoch 1 as "best", because Ultralytics' fitness is computed on
the *synthetic* validation split.

### A note on validation metrics

The bundled validation set is synthetic, and its mAP does **not** predict real-world quality.
On the nano variant, pushing synthetic pose mAP from 0.9448 to 0.9573 produced no measurable
real improvement (paired Wilcoxon on the held-out set, p = 0.83). Judge changes on real data.
...
| file | description |
|---|---|
| `pallet_yolo26m_pose_ft.pt` | the model (47.6 MB) |
