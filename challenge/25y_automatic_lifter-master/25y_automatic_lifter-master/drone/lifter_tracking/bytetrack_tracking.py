...
from cjm_byte_track.core import BYTETracker  # ByteTrack
...
QUEUE_MAXSIZE  = 10
FRAME_RATE     = 30  # ByteTrack frame_rate 가정
# ====================================
...
# ByteTrack 설정 (cjm-byte-track 방식)
tracker = BYTETracker(
    track_thresh=0.5,
    track_buffer=30,
    match_thresh=0.8,
...
try:
    while True:
        ...
        # ByteTrack 입력용 dets: [x1, y1, x2, y2, score]
        bytetracker_inputs = []
        ...
        for box in det.boxes:
            ...
            if cls == lifter_class_id:
                # ByteTrack용 박스
                bytetracker_inputs.append([x1, y1, x2, y2, conf])
        ...
        # ---------- ByteTrack 업데이트 ----------
        if len(bytetracker_inputs) > 0:
            dets_np = np.asarray(bytetracker_inputs, dtype=np.float32)
        else:
            dets_np = np.empty((0, 5), dtype=np.float32)

        online_targets = tracker.update(
            output_results=dets_np,
        ...
        if target_box:
            ...
            yaw, fb = 0, 0  # YOLO detection 없으면 정지
        ...
        cv2.imshow("Drone View (YOLO + ByteTrack ID)", frame)
